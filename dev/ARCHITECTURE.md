...
| File | Purpose |
|---|---|
| **`out/extension/debugger/watchPanel.js`** | `WatchPanelProvider` — VS Code `WebviewViewProvider` for the Wolfbook Watch sidebar. Manages the watch list state, `_lastEvalSel` cache, debug-active flag, and breakpoint list. Posts messages to the webview (`update`, `clear`, `evalSelResult`, `evalSelSpinner`, `evalSelError`, `setDebugActive`, `initWatchList`). Receives messages from the webview (`addWatch`, `removeWatch`, `refresh`, `debugCommand`, `removeBreakpoint`, `clearBreakpoints`, `openInEditor`, `evalSelClear`, `openFile`, `scriptLoaded`). Exposes `addWatchExternal(name)` for the `Cmd+Shift+W` command path. |
| **`out/extension/debugger/watchPanel.webview.js`** | Webview script that runs inside the Watch sidebar. Renders the watch table, debug control buttons, breakpoints list, and Evaluate Selection result area. Implements `setDebugging(active)` which hides/shows the breakpoints section and toggles button states. Includes client-side WL syntax validation (`validateWLSyntax`) for the watch input field. Handles the `data-open-file` anchor click → `openFile` postMessage for large-result links. |
| **`out/extension/editor/evaluateSelection.js`** | `Cmd+Shift+E` evaluate-selection feature and `Cmd+Shift+W` add-to-watch feature. Registered via `evalSel.register(context, getController, watchPanel)` from `extension.js`. Key functions: `evaluateSelection()` — gets selection, blocks if debug active, sends to kernel via `Dialog[]` or direct sub, renders result in watch panel; `_renderResult()` — formats output HTML, truncates results >60 KB and writes full HTML to a temp file; `addSelectionToWatch()` — validates syntax with `_validateWLSyntax()`, then calls `watchPanel.addWatchExternal()`; `_pickFormat()` — status-bar format picker. |
| **`out/extension/client/index-with-messaging.js`** | Renderer entry point (~1070 lines). Exports `activate(context)`. Imports from `renderer-css.js` and `renderer-highlight.js`. Contains: splash screen, state vars, message listener, wexprToInputForm, dialog widget, renderOutputItem (expand/format/copy/KaTeX buttons, height sentinel), disposeOutputItem. |
