...
export function activate(context) {
    ...
    const disposables = {};
    // outputId -> element root, so disposeOutputItem can release WebGL contexts
    // held by any interactive 3D viewers mounted inside it.
    const wl3dRoots = {};
    ...
    // Cached wl3d-viewer module handle, so a Manipulate update can ask whether
    // its result region currently hosts a live viewer without an await.
    let _wl3dMod = null;
    // Same, for the 2D hover callout: a Manipulate frame must be able to hide a
    // live callout before the <img> it points at is replaced.
    let _wl2dMod = null;
    const _attachPlotTooltips = (root) => {
        const imgs = root.querySelectorAll('img[data-wl-plot-src]:not([data-wl-mesh-src])');
        if (!imgs.length) return;
        import('./wl2d-tooltip.js')
            .then(m => { _wl2dMod = m; imgs.forEach(i => m.attachPlotTooltip(i)); })
            .catch(() => {});
    };
    // Drag-to-resize. The committed size is written back into the stored output
    // HTML by the extension host, so it survives save/reload; a plot inside a
    // Manipulate result has no stable output id and stays session-only.
    let _wlResizeMod = null;
    // Persisting a size replaces the cell output, which tears the whole output
    // DOM down. Without this a plot the reader had rotated would snap back to a
    // static image at its default angle, so the camera is stashed by the stable
    // data-output-id (the header survives the patch) and restored on remount.
    const _wl3dCamStash = Object.create(null);
    const _stashCamera = (box, outputId) => {
        if (!_wl3dMod || !outputId) return;
        try {
            const entry = _wl3dMod.findViewerIn(box);
            if (!entry || !entry.viewer) return;
            const v = entry.viewer;
            _wl3dCamStash[outputId] = {
    ...
    const _restoreCamera = (viewer, outputId) => {
        const s = outputId && _wl3dCamStash[outputId];
        if (!s) return;
        delete _wl3dCamStash[outputId];
        try {
    ...
    const _attachResize = (box, img, root) => {
        import('./wl-img-resize.js')
            .then(m => {
                _wlResizeMod = m;
                const blk = img.closest('.wl-output-block');
                const hdr = blk ? blk.querySelector('.wl-output-header') : null;
                const outputId = hdr ? hdr.getAttribute('data-output-id') : null;
                const manip = img.closest('.wl-manip-result');
                let commit = null;
                if (manip) {
                    // A Manipulate frame is rebuilt on every slider tick and has
                    // no stable output id, so the chosen width is remembered on
                    // the container and re-applied to each new frame.
                    commit = (width) => {
                        if (width) manip.setAttribute('data-wl-manip-width', String(width));
                        else manip.removeAttribute('data-wl-manip-width');
                    };
                } else if (outputId && context && context.postMessage) {
                    const imgs = blk.querySelectorAll(
                        'img.vscode-wolfram-svg-output, img.vscode-wolfram-png-output');
                    const imgIndex = Array.prototype.indexOf.call(imgs, img);
                    commit = (width, height) => {
                        _stashCamera(box, outputId);
                        try {
                            context.postMessage({
                                type: 'resize-output', outputId, imgIndex, width, height });
                        } catch (_) {}
                    };
                }
                const api = m.makeResizable(box, img, commit);
                const keep = manip ? parseFloat(manip.getAttribute('data-wl-manip-width')) : 0;
                if (api && keep > 0) api.applyWidth(keep);
                void root;
            })
            .catch(() => {});
    };
    const _meshSrcIn = (html) => {
        const m = /data-wl-mesh-src="([^"]+)"/.exec(html || '');
        return m ? m[1] : null;
    };

    // Replace a Manipulate result wholesale. Any viewer inside it must be
    // disposed FIRST — dropping the element alone leaves its WebGL context
    // alive, and a scrubbed slider would exhaust the browser's ~16 in seconds.
    function _replaceManip(t, msg) {
        if (_wl3dMod) { try { _wl3dMod.disposeViewersIn(t); } catch (_) {} }
        if (_wl2dMod) { try { _wl2dMod.hideTooltipsIn(t); } catch (_) {} }
    ...
    function _applyManipResult(msg) {
        ...
        targets.forEach(t => {
            ...
            // Replacing the HTML would tear down the WebGL context and snap the
            ...
            if (liveEntry) {
                ...
                if (nextSrc) {
                    fetch(nextSrc)
                        .then(r => r.json())
                        .then(raw => {
                            liveEntry.viewer.updateGeometry(_wl3dMod.parseScene(raw));
                            // keep the hidden fallback image in step, so if the
                            // mesh ever fails it shows this frame, not a stale one
                            const poster = liveEntry.host.querySelector('img[data-wl-mesh-src]');
                            const nextPng = /<img[^>]*\ssrc="([^"]+)"/.exec(msg.html || '');
                            if (poster && nextPng) {
                                poster.src = nextPng[1];
                                poster.setAttribute('data-wl-mesh-src', nextSrc);
                            }
                        })
                        .catch(() => { _replaceManip(t, msg); });
            ...
            // the reader picked before the slider moved.
            t.querySelectorAll('img.vscode-wolfram-svg-output, img.vscode-wolfram-png-output')
                .forEach(img => {
                    if (img.closest('.wl-img-box')) return;
                    const box = document.createElement('div');
                    box.className = 'wl-img-box';
                    box.style.cssText = 'position:relative;display:inline-block;max-width:100%;';
                    img.parentNode.insertBefore(box, img);
                    box.appendChild(img);
                    _attachResize(box, img, t);
                });
            const meshImgs = t.querySelectorAll('img[data-wl-mesh-src]');
            if (meshImgs.length) {
            ...
            if (texDivs.length && window.katex) {
                texDivs.forEach(d => {
                    try {
                        d.innerHTML = window.katex.renderToString(
                            d.getAttribute('data-tex-src') || '', KATEX_RENDER_OPTIONS);
                    } catch (_) {}
                });
            }
        });
    ...
    context.onDidReceiveMessage(msg => {
        if (msg && msg.type === 'manipulate-result') { _applyManipResult(msg); return; }
    ...
    return {
        renderOutputItem(outputItem, element) {
            ...
            outputHeaders.forEach(header => {
                ...
                // named so the 3D viewer can put its own controls in this row
                group.className = 'wl-output-btns';
                ...
                const formats = isGraphics
                    ? [['WL', 'InputForm'], ['SVG', 'SVG'], ['3D', 'WL3D'], ['TikZ', 'SVGSrc']]
                ...
                formats.forEach(([label, fmtKey]) => {
                    ...
                    b.title = fmtKey === 'TXT'
                            ...
                            : fmtKey === 'WL3D'         ? 'Interactive 3D — drag to rotate (Graphics3D only)'
            ...
            element.querySelectorAll('img.vscode-wolfram-svg-output, img.vscode-wolfram-png-output').forEach(img => {
                ...
                _attachResize(wrapper, img, element);
            });
            // ---- Manipulate sliders ----
            // The kernel emits real <input type="range"> controls; moving one
            // re-evaluates the body and patches ONLY .wl-manip-result, so the
            // slider under the user's finger is never rebuilt mid-drag.
            {
                const sliders = element.querySelectorAll('input.wl-manip-slider');
                if (sliders.length) {
                    ...
                    const readAll = (manipId) => {
                        ...
                        element.querySelectorAll(
                            'input.wl-manip-slider[data-manip-id="' + manipId + '"]'
                        ).forEach(x => { sets[x.getAttribute('data-var')] = Number(x.value); });
                        return sets;
                    };
                    sliders.forEach(sl => {
                        const manipId = sl.getAttribute('data-manip-id');
                        const varName = sl.getAttribute('data-var');
                        const out = element.querySelector(
                            '.wl-manip-value[data-manip-id="' + manipId + '"][data-var="' + varName + '"]');
                        sl.addEventListener('input', () => {
                            if (out) out.textContent = sl.value;      // instant feedback
                            clearTimeout(timers[manipId]);
                            timers[manipId] = setTimeout(() => {
                                if (context && context.postMessage) {
                                    try {
            ...
            // ---- 2D hover coordinate callout ----
            // resources/wb2d.wl ships the sampled curves beside the SVG/PNG;
            // the picture stays exactly as it was and only gains a callout on
            // hover. Images that carry a 3D mesh are skipped — that gesture is
            // already spoken for.
            _attachPlotTooltips(element);

            // ---- Interactive 3D (the "3D" output format) ----
            // resources/wb3d.wl hangs data-wl-mesh-src on the PNG when the user
            // picks WL3D. Swap in a three.js canvas; the <img> stays as the
            // fallback, so a missing/broken mesh just leaves today's picture.
            {
                const meshImgs = element.querySelectorAll('img[data-wl-mesh-src]');
                if (meshImgs.length) {
                    const _doc = element.ownerDocument || document;
                    const _dark = !(_doc.body && _doc.body.classList.contains('vscode-light'));
                    wl3dRoots[currentOutputId] = element;
                    import('./wl3d-viewer.js')
                        .then(m => {
                            // Cache the module here too, not just on the Manipulate
                            // path: without it a resize could not find the live
                            // viewer, so the camera was never stashed and a rotated
                            // plot snapped back to its default angle.
                            _wl3dMod = m;
                            meshImgs.forEach(img => {
                                // Put the viewer's controls in the output header
                                // beside WL / SVG / 3D / TikZ rather than floating
                                // them over the picture.
                                const blk = img.closest('.wl-output-block');
                                const grp = blk ? blk.querySelector('.wl-output-btns') : null;
                                // Once the viewer is live the output really IS 3D,
                                // so move the highlight off SVG and onto the 3D button.
                                const markActive3D = () => {
                                    if (!grp) return;
                                    grp.querySelectorAll('button[data-fmt-key]').forEach(b => {
                                        const on = b.getAttribute('data-fmt-key') === 'WL3D';
                                        b.style.cssText = BTN_BASE + (on ? BTN_ACTIVE : '');
                                    });
                                };
                                // A resize replaced the output; if this plot was a
                                // live rotated view a moment ago, bring it back
                                // exactly as it was rather than as a flat picture.
                                const hdr3d = blk ? blk.querySelector('.wl-output-header') : null;
                                const oid3d = hdr3d ? hdr3d.getAttribute('data-output-id') : null;
                                const stashed = oid3d && _wl3dCamStash[oid3d];
                                const opts = {
                                    dark: _dark,
                                    toolbar: grp,
                                    toolbarBtnCss: BTN_BASE,
                                    onReady: (viewer) => {
                                        markActive3D();
                                        if (viewer) _restoreCamera(viewer, oid3d);
                                    },
                                };
                                if (stashed || img.getAttribute('data-wl-3d-open') === '1') {
                                    // user picked the 3D format — open it now
                                    m.mountViewer(img, opts);
                                } else {
                                    // ordinary static image: stays a picture until
                                    // someone grabs it, then rotates from that drag
                                    m.attachLazy3D(img, opts);
                                }
                            });
                        })
                        .catch(e => console.warn('[WolframRenderer] 3D viewer failed to load:', e));
                }
            }
        ...
        disposeOutputItem(outputId) {
            ...
            // Release WebGL contexts before anything else — browsers cap them
            // and silently drop the oldest, which would blank a live viewer.
            const _releaseGl = (roots) => {
                if (!roots.length) return;
                import('./wl3d-viewer.js')
                    .then(m => roots.forEach(r => { try { m.disposeViewersIn(r); } catch (_) {} }))
                    .catch(() => {});
            };
            if (typeof outputId === 'string') {
                const root = wl3dRoots[outputId];
                if (root) { delete wl3dRoots[outputId]; _releaseGl([root]); }
                const disposable = disposables[outputId];
                disposable?.disconnect();
                delete disposables[outputId];
            } else {
                const roots = Object.values(wl3dRoots);
                Object.keys(wl3dRoots).forEach(k => delete wl3dRoots[k]);
                _releaseGl(roots);
